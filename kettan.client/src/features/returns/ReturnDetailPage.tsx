        ...
        <Box sx={{ 
          ...
          <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1.5fr', gap: 1.5 }}>
            ...
            <Typography sx={{ fontSize: 13, fontWeight: 800, textAlign: 'right', color: '#3E2723', fontFamily: 'monospace' }}>
              {(row.transactionCode || '').replace(/^[A-Z]+-/, '')}
            </Typography>
...
export function ReturnDetailPage() {
  ...
  return (
    <>
      <LoadingOverlay open={isLoading || isSaving} />
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: '400px 1fr', gap: 2.5, alignItems: 'start' }}>
        ...
        <Paper sx={{ p: 0, borderRadius: '14px', border: '1px solid', borderColor: 'divider', overflow: 'hidden' }} elevation={0}>
          ...
          <Box sx={{ 
              ...
              <Typography sx={{ fontSize: 20, fontWeight: 700, color: 'text.primary', letterSpacing: '-0.01em' }}>{(row.transactionCode || '').replace(/^[A-Z]+-/, '')}</Typography>
              <Typography sx={{ fontSize: 13, fontWeight: 500, color: 'text.secondary', mt: -0.5 }}>From Order {String(row.orderTransactionCode || row.orderId).replace(/^[A-Z]+-/, '')}</Typography>
          </Box>
                ...
                <Box sx={{ p: 2.5, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 2 }}>
                    ...
                    {isHq && (
                        <Button 
                            variant="contained" 
                            size="small"
                            onClick={() => navigate({ to: '/orders/new', search: { fromReturnId: String(row.returnId) } })}
                            sx={{ 
                                whiteSpace: 'nowrap',
                                bgcolor: '#6B4C2A',
      ...
      <WorkflowActionModal
        ...
        description="Confirm that the return items have been handed off to the pickup vehicle."
        inputLabel="Remarks (Optional)"
        inputPlaceholder="Any notes about the handoff..."
        confirmLabel="Confirm Handoff"
        severity="info"
        loading={isSaving}
        onConfirm={(remarks) => void withSave(() => confirmReturnDispatch(row.returnId, remarks || undefined).then(() => {}))}
        onCancel={() => setDialog(null)}
      />
      <WorkflowActionModal
        open={dialog === 'arrival'}
        title="Confirm Return Arrived at HQ"
        description="Confirm that the returned items have arrived at the HQ warehouse."
        inputLabel="Remarks (Optional)"
        inputPlaceholder="Any notes about the arrival condition..."
        confirmLabel="Confirm Arrival"
        severity="success"
        loading={isSaving}
        onConfirm={(remarks) => void withSave(() => confirmReturnArrival(row.returnId, remarks || undefined).then(() => {}))}
        onCancel={() => setDialog(null)}
      />
      <WorkflowActionModal
        open={dialog === 'startInspect'}
        title="Start Inspection"
        description="Begin inspecting the returned items. You can add initial inspection notes below."
        inputLabel="Remarks (Optional)"
        inputPlaceholder="Any initial inspection notes..."
        confirmLabel="Start Inspection"
        severity="info"
        loading={isSaving}
        onConfirm={(remarks) => void withSave(() => startReturnInspection(row.returnId, remarks || undefined).then(() => {}))}
        onCancel={() => setDialog(null)}
      />
