...
import { fetchBranchPerformanceDetail, type BranchPerformanceDetailDto } from '../reportsApi';

interface Props { startDate: string; endDate: string; }

const EMPTY: BranchPerformanceDetailDto = {
...
function MetricBar({
  ...
  const display = good === 'low'
    ? Math.max(0, 100 - value * 2)   // invert for display bar width
    : value;
...
export function BranchMyPerformanceTab({ startDate, endDate }: Props) {
  const [data, setData] = useState<BranchPerformanceDetailDto>(EMPTY);
    ...
    <Box sx={{ opacity: loading ? 0.6 : 1, transition: 'opacity 0.2s', display: 'flex', flexDirection: 'column', gap: 2.5 }}>
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: '280px 1fr' }, gap: 2.5 }}>
        ...
        <Card elevation={0} sx={{ border: '1px solid', borderColor: 'divider', borderRadius: '14px', p: 2.5, bgcolor: 'background.paper', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1.5 }}>
            ...
            <Typography sx={{ fontSize: 15, fontWeight: 700, color: 'text.primary' }}>My Score</Typography>
          </Box>
          <ScoreGauge score={data.weightedScore} />
          {data.totalBranches > 0 && (
            <Box sx={{ mt: 1.5, textAlign: 'center' }}>
              <Typography sx={{ fontSize: 13, color: 'text.secondary' }}>
                Ranked{' '}
                <strong style={{ color: data.rankInChain === 1 ? '#C9A84C' : '#374151' }}>
                  #{data.rankInChain}
                </strong>
                {' '}of {data.totalBranches} branches
              </Typography>
            </Box>
          )}
        </Card>
        ...
        <Card elevation={0} sx={{ border: '1px solid', borderColor: 'divider', borderRadius: '14px', p: 2.5, bgcolor: 'background.paper' }}>
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            <MetricBar label="Fulfillment Rate" value={data.fulfillmentRate} weight="30%" good="high" color="#6B4C2A" />
            <MetricBar label="Return Rate" value={data.returnRate} weight="20%" good="low" color="#DC2626" />
            <MetricBar label="Delivery Speed" value={data.deliverySpeedHrs} unit="hrs" weight="25%" good="low" color="#2563EB" />
            <MetricBar label="Stock Accuracy" value={data.stockAccuracy} weight="25%" good="high" color="#546B3F" />
          </Box>
        </Card>
      </Box>
      ...
      <Card elevation={0} sx={{ border: '1px solid', borderColor: 'divider', borderRadius: '14px', p: 2, bgcolor: 'rgba(107,76,42,0.03)' }}>
        <Typography sx={{ fontSize: 12, color: 'text.secondary' }}>
          <strong style={{ color: '#6B4C2A' }}>Weighted Score Formula:</strong>{' '}
          (Fulfillment Rate × 30%) + (Return Score × 20%) + (Speed Score × 25%) + (Stock Accuracy × 25%).
          Return and Speed metrics are inverted — lower is better.
        </Typography>
      </Card>
    </Box>
  );
